...
echo 'Below we will list any important system updates that should be installed. We are checking for updates now.<BR><BR>';
...
if (strval($system_data["core_version"])!=strval($data["version"])){
  ...
  echo '<div style="position: absolute;bottom: 10px;right: 60px;"><a href="javascript:modl(\''.$settings["admin_url"].'?page=update_install&ui_nostyle=true\');">Refresh install</a> (runs like a normal update)</div>';
}
